...
grant select, insert, update, references on "repsic211_grupo_personas_calculada" to repsic_user;
...
grant select, insert, update, references on "repsic211_personas_calculada" to repsic_user;
...
grant select, insert, update, references on "repsic211_supervision_calculada" to repsic_user;
grant all on "repsic211_supervision_calculada" to repsic_owner;



-- conss
alter table "repsic211_grupo_personas_calculada" add constraint "operativo<>''" check ("operativo"<>'');
alter table "repsic211_grupo_personas_calculada" add constraint "id_caso<>''" check ("id_caso"<>'');
alter table "repsic211_personas_calculada" add constraint "operativo<>''" check ("operativo"<>'');
alter table "repsic211_personas_calculada" add constraint "id_caso<>''" check ("id_caso"<>'');
-- FKs
...
$SQL_ENANCE$;
----
INSERT INTO "repsic211_grupo_personas_calculada" ("operativo","id_caso") SELECT "operativo","id_caso" FROM "grupo_personas";
----
INSERT INTO "repsic211_personas_calculada" ("operativo","id_caso","p0") SELECT "operativo","id_caso","p0" FROM "personas";
----
INSERT INTO "repsic211_supervision_calculada" ("recorrido") SELECT "recorrido" FROM "supervision";
----
CREATE OR REPLACE FUNCTION repsic.gen_fun_var_calc() RETURNS TEXT
  ...
  cant_refe = personas_agg.cant_refe
            ...
            count(nullif(case when null2zero(personas.p1) = 1 then true else null end,false)) as cant_refe
                       ...
                       WHERE repsic211_grupo_personas_calculada.PK = personas.fPK
                   ) personas_agg

    WHERE "grupo_personas"."id_caso"="repsic211_grupo_personas_calculada"."id_caso";
  ...
  cant_per = personas_agg.cant_per
            ...
            count("p0") as cant_per
                       FROM "grupo_personas" join "personas"ON "grupo_personas"."id_caso"="personas"."id_caso"
                       WHERE repsic211_grupo_personas_calculada.PK = "grupo_personas".PK
    ...
    WHERE "personas"."id_caso"="repsic211_personas_calculada"."id_caso" AND "personas"."p0"="repsic211_personas_calculada"."p0";
        ...
        SET 
            ...
            FROM (
                     SELECT
                      ...
                      FROM "grupo_personas"
                      WHERE repsic211_supervision_calculada
