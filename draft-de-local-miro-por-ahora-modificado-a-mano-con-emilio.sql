...
grant select, insert, update, references on "repsic212_grupo_personas_calculada" to repsic_user;
grant all on "repsic212_grupo_personas_calculada" to repsic_owner;
...
grant select, insert, update, references on "repsic212_personas_calculada" to repsic_user;
grant all on "repsic212_personas_calculada" to repsic_owner;
...
grant select, insert, update, references on "repsic212_supervision_calculada" to repsic_user;
grant all on "repsic212_supervision_calculada" to repsic_owner;



-- conss
alter table "repsic212_grupo_personas_calculada" add constraint "operativo<>''" check ("operativo"<>'');
alter table "repsic212_grupo_personas_calculada" add constraint "id_caso<>''" check ("id_caso"<>'');
alter table "repsic212_personas_calculada" add constraint "operativo<>''" check ("operativo"<>'');
alter table "repsic212_personas_calculada" add constraint "id_caso<>''" check ("id_caso"<>'');
-- FKs
...
alter table "repsic212_personas_calculada" add constraint "repsic212_personas_calculada personas REL" foreign key ("operativo", "id_caso", "p0") references "personas" ("operativo", "id_caso", "p0")  on delete cascade on update cascade;
alter table "repsic212_supervision_calculada" add constraint "repsic212_supervision_calculada supervision REL" foreign key ("recorrido") references "supervision" ("recorrido")  on delete cascade on update cascade;
-- index
create index "operativo,id_caso 4 repsic212_grupo_personas_calculada IDX" ON "repsic212_grupo_personas_calculada" ("operativo", "id_caso");
create index "operativo,id_caso,p0 4 repsic212_personas_calculada IDX" ON "repsic212_personas_calculada" ("operativo", "id_caso", "p0");
create index "recorrido 4 repsic212_supervision_calculada IDX" ON "repsic212_supervision_calculada" ("recorrido");
-- functions

----
do $SQL_ENANCE$
 begin
PERFORM enance_table('repsic212_grupo_personas_calculada','operativo,id_caso');
PERFORM enance_table('repsic212_personas_calculada','operativo,id_caso,p0');
PERFORM enance_table('repsic212_supervision_calculada','recorrido');
end
$SQL_ENANCE$;
----
INSERT INTO "repsic212_grupo_personas_calculada" ("operativo","id_caso") SELECT "operativo","id_caso" FROM "grupo_personas";
----
INSERT INTO "repsic212_personas_calculada" ("operativo","id_caso","p0") SELECT "operativo","id_caso","p0" FROM "personas";
----
INSERT INTO "repsic212_supervision_calculada" ("recorrido") SELECT "recorrido" FROM "supervision";
----
CREATE OR REPLACE FUNCTION repsic.gen_fun_var_calc() RETURNS TEXT
      ...
      UPDATE repsic212_grupo_personas_calculada
        SET 
  ...
  cant_refe = personas_agg.cant_refe
            ...
            count(nullif(case when null2zero(personas.p1) = 1 then true else null end,false)) as cant_refe
                       ...
                       WHERE repsic212_grupo_personas_calculada.PK = personas.fPK
                   ) personas_agg

    WHERE "grupo_personas"."id_caso"="repsic212_grupo_personas_calculada"."id_caso";
  ...
  cant_per = personas_agg.cant_per
            ...
            count("p0") as cant_per
                       FROM "grupo_personas" join "personas"ON "grupo_personas"."id_caso"="personas"."id_caso"
                       WHERE repsic212_grupo_personas_calculada.PK = "grupo_personas".PK

                   ) personas_agg
            
    WHERE "grupo_personas"."id_caso"="repsic212_grupo_personas_calculada"."id_caso";

      UPDATE repsic212_personas_calculada
        SET 
    ...
    WHERE "personas"."id_caso"="repsic212_personas_calculada"."id_caso" AND "personas"."p0"="repsic212_personas_calculada"."p0";

      UPDATE repsic212_supervision_calculada
        SET 
            ...
            FROM (
                     SELECT
                      ...
                      FROM "grupo_personas"
                      WHERE repsic212_supervision_calculada
                      
                   ) grupo_personas_agg        

    WHERE "supervision"."recorrido"="repsic212_supervision_calculada"."recorrido";
          RETURN 'OK';
        END;
        $BODY$;
----
...
----
        ...
        UPDATE operativos SET calculada=now()::timestamp(0) WHERE operativo='repsic212';
        UPDATE tabla_datos SET generada=now()::timestamp(0) WHERE operativo='repsic212' AND tipo='calculada';
----
end
