...
drop table if exists "repsic_251_grupo_personas_calculada";
drop table if exists "repsic_251_personas_calculada";
drop table if exists "repsic_251_coordinacion_calculada";
...
--set role repsic251_admin;

create table "repsic_251_grupo_personas_calculada" (
  operativo             text,
  id_caso               text,
  "cant_0a14_mu" bigint, 
...
--grant all on "repsic_251_grupo_personas_calculada" to "repsic251_owner";

create table "repsic_251_personas_calculada" (
  "operativo" text,
  "id_caso" text,
  "persona" bigint, 
  "sexor" bigint,
...
--grant select, insert, update, delete, references on "repsic_251_personas_calculada" to "repsic251_admin";
...
-- conss
alter table "repsic_251_grupo_personas_calculada" add constraint "operativo<>''" check ("operativo"<>'');
alter table "repsic_251_grupo_personas_calculada" add constraint "id_caso<>''" check ("id_caso"<>'');
alter table "repsic_251_personas_calculada" add constraint "operativo<>''" check ("operativo"<>'');
alter table "repsic_251_personas_calculada" add constraint "id_caso<>''" check ("id_caso"<>'');
...
BEGIN
  ...
  ----
    ...
    UPDATE repsic_251_grupo_personas_calculada
      ...
      WHERE "grupo_personas"."operativo"="repsic_251_grupo_personas_calculada"."operativo" AND "grupo_personas"."id_caso"="repsic_251_grupo_personas_calculada"."id_caso" AND "grupo_personas"."operativo"=p_operativo AND "grupo_personas"."id_caso"=p_id_caso;
    UPDATE repsic_251_personas_calculada
      SET 
          sexor = null2zero(referente.sc3), -- VER QUIZAS QUIERA EL NULL!!!
          edadr = null2zero(referente.sc4)
    FROM personas inner join grupo_personas using (operativo, id_caso) inner join repsic_251_grupo_personas_calculada using (operativo, id_caso)
        LEFT JOIN (
    ...
    WHERE "personas"."operativo"="repsic_251_personas_calculada"."operativo" AND "personas"."id_caso"="repsic_251_personas_calculada"."id_caso" AND "personas"."persona"="repsic_251_personas_calculada"."persona" 
      AND "personas"."operativo"=p_operativo AND "personas"."id_caso"=p_id_caso;
    UPDATE repsic_251_coordinacion_calculada
      SET 
...
-----
select update_varcal('REPSIC_251');
UPDATE operativos SET calculada=now()::timestamp(0) WHERE operativo='REPSIC_251';
UPDATE tabla_datos SET generada=now()::timestamp(0) WHERE operativo='REPSIC_251' AND tipo='calculada';
-----